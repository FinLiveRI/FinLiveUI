...
const mockGetGroup= async(query: any) => {
  ...
  data.forEach((animal_data: any) => {
    ...
    group_data.feed = animal_data.feed.map((obj: any, index: number) => group_data.feed[index]?.value 
      ? {
    ...
    group_data.weight = animal_data.weight.map((obj: any, index: number) => group_data.weight[index]?.weight 
      ? {
          timestamp: obj.timestamp, 
    ...
    group_data.milk = animal_data.milk.map((obj: any, index: number) => group_data.milk[index]?.totalmilk 
      ? {
          timestamp: obj.timestamp, 
  ...
  const insentec = group_data.feed.map((obj: any) => ({...obj, timestamp: obj.timestamp, value: obj.value - Math.floor(Math.random() * 30)}));
  const robot = group_data.feed.map((obj: any) => ({...obj, timestamp: obj.timestamp, value: obj.value - Math.floor(Math.random() * 10)}));
