...
const mockGetGroup= async(query: any) => {
  ...
  data.forEach((animal_data: any) => {
    ...
    group_data.feed = animal_data.feed.map((obj: any, index: number) => group_data.feed[index]?.value ? {timestamp: obj.timestamp, value: group_data.feed[index].value + Number(obj.value)} : obj);
    group_data.weight = animal_data.weight.map((obj: any, index: number) => group_data.weight[index]?.weight ? {timestamp: obj.timestamp, weight: group_data.weight[index].weight + Number(obj.weight)} : obj);
    group_data.milk = animal_data.milk.map((obj: any, index: number) => group_data.milk[index]?.totalmilk ? {timestamp: obj.timestamp, totalmilk: group_data.milk[index].totalmilk + Number(obj.totalmilk)} : obj);
  })

  const insentec = group_data.feed.map((obj: any) => ({timestamp: obj.timestamp, value: obj.value - Math.floor(Math.random() * 30)}));
  const robot = group_data.feed.map((obj: any) => ({timestamp: obj.timestamp, value: obj.value - Math.floor(Math.random() * 10)}));
