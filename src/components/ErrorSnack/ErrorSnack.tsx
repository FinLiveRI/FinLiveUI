  ...
  return (
    <Snackbar open={props.open} onClose={props.handleClose}>
      <Alert onClose={props.handleClose} severity="error">
        <AlertTitle>
          <FormattedMessage description="Error snack title" defaultMessage="Error" />
        </AlertTitle>
        {
          props.error?.response?.data?.message
          || props.error?.message
          || <FormattedMessage description="Generic error message" defaultMessage="Something went wrong" />
        }
      </Alert>
    </Snackbar>
  )
